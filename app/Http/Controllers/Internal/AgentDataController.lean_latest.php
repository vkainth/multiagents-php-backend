...
class AgentDataController extends Controller
{
    ...
    public function featuredListings(string $slug, Request $req): JsonResponse
    {
        ...
        if ($withSuite && $status === 'Active') {
            $q->where(function ($sq) {
                $sq->where('kitchens', '>=', 2)
                   ->orWhere('basement', 'LIKE', '%Separate Entry%')
                   ->orWhere('basement', 'LIKE', '%Exterior Entry%');
            });
        }
        ...
        if ($coachHome && $status === 'Active') {
            $q->where(function ($sq) {
                $sq->whereRaw("LOWER(remarks) LIKE '%coach home%'")
                   ->orWhereRaw("LOWER(remarks) LIKE '%carriage house%'")
                   ->orWhereRaw("LOWER(remarks) LIKE '%coach house%'");
            });
        }
        if ($lanewayHouse && $status === 'Active') {
            $q->where(function ($sq) {
                $sq->whereRaw("LOWER(remarks) LIKE '%laneway house%'")
                   ->orWhereRaw("LOWER(remarks) LIKE '%laneway home%'");
            });
        }
        if ($legalSuite && $status === 'Active') {
            $q->whereRaw("LOWER(remarks) LIKE '%legal suite%'");
        }
        ...
        $data = \Illuminate\Support\Facades\Cache::remember('rc_rs_' . $limit, 900, function () use ($limit) {
            ...
            try {
                $rows = \Illuminate\Support\Facades\DB::connection('mysql_mlsr')
                    ...
                    ->select(['subarea', 'city', 'class', 'soldprice_2', 'sold_date'])
                    ->limit($limit)
                    ->get();
                $out = [];
                foreach ($rows as $r) {
    ...
    {
        $days = max(7, min(365, (int) $req->query('days', 60)));
        $data = \Illuminate\Support\Facades\Cache::remember('rc_ov_' . $days, 1800, function () use ($days) {
            $cities = $this->residencityMetroCities();
            $since  = now()->subDays($days)->format('Y-m-d');
            $db     = \Illuminate\Support\Facades\DB::connection('mysql_mlsr');
            $ss = null; $stl = null; $ac = 0;
            try {
                $ss = $db->table('mlsr_listings_master')
                    ->whereIn('city', $cities)->where('status', 'Sold')
                    ->where('sold_date', '>=', $since)
                    ->whereNotNull('soldprice_2')->where('soldprice_2', '>', 0)
                    ->selectRaw('COUNT(*) as cnt, ROUND(AVG(soldprice_2)) as avg_price, ROUND(AVG(DATEDIFF(sold_date, list_date))) as avg_dom')
                    ->first();
            } catch (\Throwable $e) {}
            try {
                $stl = $db->table('mlsr_listings_master')
                    ->whereIn('city', $cities)->where('status', 'Sold')
                    ->where('sold_date', '>=', $since)
                    ->whereNotNull('soldprice_2')->where('soldprice_2', '>', 0)
                    ->whereNotNull('listprice_2')->where('listprice_2', '>', 0)
                    ->selectRaw('ROUND(AVG(soldprice_2 / listprice_2 * 100), 2) as stl')
                    ->first();
            } catch (\Throwable $e) {}
            try {
            ...
            return [
                'sold_count'     => (int) ($ss->cnt ?? 0),
                'active_count'   => (int) $ac,
                'avg_sold_price' => (int) ($ss->avg_price ?? 0),
                'avg_dom'        => (int) ($ss->avg_dom ?? 0),
                'sold_to_list'   => (float) ($stl->stl ?? 0),
                'days'           => $days,
            ];
        });
        return response()->json($data);
    ...
    public function residencityTrends(\Illuminate\Http\Request $req): \Illuminate\Http\JsonResponse
    {
        ...
        $data = \Illuminate\Support\Facades\Cache::remember('rc_tr_' . $days, 3600, function () use ($days) {
            ...
            foreach ($cp as $sub => $cur) {
                ...
                $entry = ['subarea' => $sub, 'city' => $cur->city, 'pct_change' => $pct, 'avg_price' => (int) $cur->avg_price, 'prior_avg' => (int) $prior->avg_price, 'sold_count' => (int) $cur->cnt];
            ...
            foreach ($ar as $sub => $ac) {
                ...
                $inv[] = ['subarea' => $sub, 'city' => $ac->city, 'months_supply' => $mo, 'active_count' => (int) $ac->active_count, 'monthly_sold' => round($rate, 1), 'market_type' => $mo < 2 ? 'sellers' : ($mo < 4 ? 'balanced' : 'buyers')];
            }
            ...
            return [
                'most_active'      => $ml($ma, function ($r) { return ['subarea' => $r->subarea, 'city' => $r->city, 'sold_count' => (int) $r->sold_count, 'avg_price' => (int) $r->avg_price]; }),
                'speed_of_market'  => $ml($sp, function ($r) { return ['subarea' => $r->subarea, 'city' => $r->city, 'avg_dom' => (int) $r->avg_dom, 'sold_count' => (int) $r->sold_count]; }),
                'price_per_sqft'   => array_values(array_filter($ml($pf, function ($r) { return ['subarea' => $r->subarea, 'city' => $r->city, 'avg_ppsf' => (int) ($r->avg_ppsf ?? 0), 'sold_count' => (int) $r->sold_count]; }), function ($r) { return $r['avg_ppsf'] > 0; })),
                'price_gains'      => array_slice($gains, 0, 10),
                'price_drops'      => array_slice($drops, 0, 10),
                'sold_to_list'     => $ml($sl, function ($r) { return ['subarea' => $r->subarea, 'city' => $r->city, 'avg_stl' => (float) $r->avg_stl, 'sold_count' => (int) $r->sold_count]; }),
