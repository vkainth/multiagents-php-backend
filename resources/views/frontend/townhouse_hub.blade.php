...
@php
$metaTitle = 'Townhouses for Sale in Metro Vancouver, BC | MLS® Listings & Market Data';
$metaDesc  = 'Browse MLS® townhouses for sale across Metro Vancouver. Compare avg sold prices, days on market, and buyer\'s vs seller\'s market conditions by city. Updated daily from MLS® records.';
$canonicalUrl = 'https://www.bccondosandhomes.com/townhouses/';
@endphp
  ...
  "@graph": [
    {
      ...
      "itemListElement": [
        ...
        {"@type":"ListItem","position":2,"name":"Townhouses","item":"{{ $canonicalUrl }}"}
      ]
    },
    {
      "@type": "FAQPage",
      "mainEntity": [
        {"@type":"Question","name":"What is the average townhouse price in Metro Vancouver?","acceptedAnswer":{"@type":"Answer","text":"Average townhouse prices in Metro Vancouver vary by city. Browse individual city pages for current 30-day averages from live MLS® data updated daily."}},
        {"@type":"Question","name":"Is Metro Vancouver a buyer's or seller's market for townhouses?","acceptedAnswer":{"@type":"Answer","text":"Market conditions vary by city and neighbourhood. Check each city's townhouse market page for the current absorption rate, days on market, and market verdict based on the latest MLS® sales data."}}
      ]
    }
  ]
}
    ...
    <div class="container">
        <nav aria-label="breadcrumb" style="margin-bottom:12px;">
            <ol class="breadcrumb" style="background:none;padding:0;margin:0;font-size:13px;">
                ...
                <li class="breadcrumb-item active" style="color:#fff;">Townhouses</li>
            </ol>
        </nav>
        <h1 style="font-size:28px;font-weight:700;margin-bottom:12px;color:#fff;line-height:1.3;">Townhouses for Sale in Metro Vancouver, BC</h1>
        <p style="font-size:15px;color:rgba(255,255,255,.82);max-width:820px;line-height:1.7;margin-bottom:0;">
            Browse MLS® townhouses for sale across Metro Vancouver. Compare average sold prices, days on market,
...
<div class="container" style="padding-top:28px;padding-bottom:50px;min-height:60vh;">
    ...
    <h2 style="font-size:20px;font-weight:700;color:#2c2c2c;margin-bottom:16px;">Metro Vancouver Townhouse Market — City Comparison</h2>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <thead>
                    <tr style="background:#1a2a3a;color:#fff;text-align:left;">
                        ...
                        <th style="padding:12px 16px;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.5px;">Avg Townhouse Price</th>
                ...
                <tbody>
                    ...
                    @endphp
                    <tr style="border-top:1px solid #f0ede8;" class="th-city-row">
                        <td style="padding:12px 16px;font-weight:700;color:#2c2c2c;">
                            <a href="/townhouses/{{ $cSlug }}/" style="color:#2c2c2c;text-decoration:none;">{{ $city->label ?? $city->place }}</a>
                        </td>
                        ...
                        <td style="padding:12px 16px;color:#555;">
                            @if($cond['sold_30d'])
                            <a href="/search-listings/{{ $cSlug }}?type=Townhouse&listing_status=sold" style="color:#555;text-decoration:underline dotted;">{{ $cond['sold_30d'] }}</a>
                            @else
                            —
                            @endif
                        </td>
                        ...
                        <td style="padding:12px 16px;">
                            <a href="/townhouses/{{ $cSlug }}/" style="color:#2c6fad;font-size:12px;font-weight:600;white-space:nowrap;">View guide &rsaquo;</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <section style="background:#f7f4ef;border:1px solid #e2dbd2;border-radius:6px;padding:22px 26px;margin-bottom:28px;">
        <h2 style="font-size:18px;font-weight:700;color:#2c2c2c;margin:0 0 10px;">Find a Townhouse in Your Budget</h2>
        <p style="font-size:14px;color:#555;margin-bottom:16px;line-height:1.65;">
            Browse Metro Vancouver townhouses filtered by price range.
        </p>
        <div class="row">
            @php
            $priceTiers = [
                ['label' => 'Under $700K',    'min' => 50000,   'max' => 700000],
                ['label' => '$700K – $900K',  'min' => 700000,  'max' => 900000],
                ['label' => '$900K – $1.2M',  'min' => 900000,  'max' => 1200000],
                ['label' => '$1.2M – $1.5M',  'min' => 1200000, 'max' => 1500000],
                ['label' => '$1.5M+',         'min' => 1500000, 'max' => 20000000],
            ];
            @endphp
            ...
            <div class="col-md-2 col-sm-4 col-xs-6" style="margin-bottom:12px;">
                <div style="background:#fff;border:1px solid #e2dbd2;border-radius:5px;overflow:hidden;">
                    ...
                    <div style="padding:8px 6px;display:flex;flex-wrap:wrap;gap:4px;">
                        ...
                        <a href="/search-listings/{{ $cSlug }}?type=Townhouse&min_price={{ $tier['min'] }}&max_price={{ $tier['max'] }}" style="font-size:11px;color:#2c6fad;text-decoration:none;white-space:nowrap;">{{ $c->label ?? $c->place }}</a>
        ...
        <div style="margin-top:10px;font-size:13px;color:#666;">
            <a href="/search-listings?type=Townhouse" style="color:#2c6fad;font-weight:600;">Browse all Metro Vancouver townhouses for sale &rsaquo;</a>
            &nbsp;·&nbsp;
            @foreach($cities as $c)
            @php $cSlug = App\Helpers\Helper::enslugPlace($c->place); @endphp
            <a href="/search-listings/{{ $cSlug }}?type=Townhouse" style="margin:0 6px;color:#555;font-size:12px;">{{ $c->label ?? $c->place }}</a>
            @endforeach
        </div>
    </section>
    ...
    <section style="margin-bottom:30px;">
        <h2 style="font-size:18px;font-weight:700;color:#2c2c2c;margin-bottom:16px;">Recently Listed Townhouses</h2>
        <div class="row">
        ...
        <div style="margin-top:4px;font-size:13px;">
            <a href="/search-listings?type=Townhouse" style="color:#2c6fad;font-weight:600;">See all Metro Vancouver townhouses for sale &rsaquo;</a>
        </div>
    </section>
    @endif
    ...
    <section style="margin-bottom:30px;padding-top:20px;border-top:1px solid #eee;">
        <h2 style="font-size:18px;font-weight:700;color:#2c2c2c;margin-bottom:12px;">About the Metro Vancouver Townhouse Market</h2>
        <div style="font-size:14px;color:#444;line-height:1.8;max-width:820px;">
            <p>
                Metro Vancouver's townhouse market offers an attractive middle ground between condos and detached homes —
                typically featuring 2–4 bedrooms, private outdoor space, and attached garages. From South Surrey rowhomes
                to Burnaby strata townhouses and Langley family townhomes, BC Condos And Homes tracks all active and sold
                MLS® townhouse listings across the region.
            </p>
            <p style="margin-top:10px;">
                ...
                from actual townhouse sales over the last 30 and 90 days.
            </p>
        </div>
    </section>
    ...
    <div style="padding:16px 0;border-top:1px solid #eee;font-size:13px;color:#666;line-height:2;">
        ...
        @foreach($cities as $c)
        <a href="/townhouses/{{ App\Helpers\Helper::enslugPlace($c->place) }}/" style="margin:0 8px;color:#2c6fad;">{{ $c->label ?? $c->place }} Townhouses</a>
        @endforeach
...
<div class="container">
    @include('frontend.includes.alert_cta_strip', [
        ...
        'stripHeading'    => 'Get New Townhouse Listing Alerts',
        'stripSubtext'    => 'Be first to know when new townhouses hit the market across Metro Vancouver — updated daily from MLS®.',
        'stripSearchName' => 'Metro Vancouver Townhouse Listings',
        'stripSearchData' => json_encode(['listing_status' => 'Active', 'type' => 'Townhouse']),
        'stripModalId'    => 'thHubAlert',
    ])
</div>
...
@push('after-styles')
<style>
.th-city-row:hover { background:#fafaf8 !important; }
.th-city-row:hover td:first-child a { color:#2c6fad !important; }
</style>
@endpush
