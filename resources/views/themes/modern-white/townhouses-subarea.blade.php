  ...
  <div class="container">
    <nav aria-label="breadcrumb" style="margin-bottom:10px;">
      <ol style="list-style:none;padding:0;margin:0;display:flex;flex-wrap:wrap;gap:6px;font-size:13px;color:var(--muted);">
        ...
        <li><a href="{{ route('agent.townhouses', $agent->slug) }}" style="color:var(--muted);">Townhouses</a></li>
        <li style="margin:0 4px;">›</li>
        <li><a href="{{ route('agent.townhouses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:var(--muted);">{{ $city }}</a></li>
        <li style="margin:0 4px;">›</li>
...
<div class="container">
  ...
  <section class="section--sm" style="padding-top:36px;">
    <div style="background:#f8f8f6;border:1px solid var(--border);border-radius:6px;padding:20px;font-size:14px;color:var(--muted);margin-bottom:32px;">
      ...
      <a href="{{ route('agent.townhouses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:var(--text);text-decoration:underline;margin-left:8px;">View {{ $city }} townhouse market →</a>
    </div>
  </section>
  @endif
  ...
  <section class="section" aria-labelledby="listings-heading" style="border-top:1px solid var(--border);padding-top:40px;">
    ...
    <p style="color:var(--muted);">No active townhouse listings found in {{ $subarea }} right now.
      <a href="{{ route('agent.townhouses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:var(--text);text-decoration:underline;">Browse all {{ $city }} townhouses</a>.
    </p>
    @endif
  </section>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
      ...
      <a href="{{ route('agent.townhouses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $nbSlug]) }}" style="display:block;text-decoration:none;">
        <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:14px 18px;box-shadow:0 1px 3px rgba(0,0,0,.04);">
          ...
          <div style="font-size:12px;color:var(--accent);margin-top:4px;text-decoration:underline;">View townhouses &rsaquo;</div>
        </div>
      </a>
      @endforeach
    </div>
