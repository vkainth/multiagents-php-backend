...
<div class="container">
  <section class="section">
    <div class="about-bio-grid">
      ...
      <div>
        <p class="eyebrow mb-16">Background</p>
        @if($agent->bio)
          @foreach(explode("\n\n", $agent->bio) as $para)
  ...
  {{-- Testimonials --}}
  @if($testimonials->count() > 0)
  <section class="section" aria-labelledby="reviews-heading" style="border-top:1px solid var(--border);">
    <p class="eyebrow mb-12">Client Reviews</p>
  ...
  {{-- Contact CTA --}}
  <section class="section" aria-labelledby="contact-heading" style="border-top:1px solid var(--border);">
    <div class="grid-2" style="gap:64px;align-items:start;">
      <div>
        <p class="eyebrow mb-12">Get in Touch</p>
        <h2 id="contact-heading" class="h2 mb-16">Let's Talk</h2>
        ...
        <div class="info-box" style="margin-bottom:16px;">
          @if($agent->phone)
            <div><strong>Phone:</strong> <a href="tel:{{ $agent->phone }}" style="color:var(--text);font-weight:700;">{{ $agent->phone }}</a></div>
          @endif
          @if($agent->email)
            <div><strong>Email:</strong> <a href="mailto:{{ $agent->email }}" style="color:var(--text);font-weight:700;">{{ $agent->email }}</a></div>
          @endif
          @if($agent->brokerage)
