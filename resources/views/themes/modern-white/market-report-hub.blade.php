  ...
  <section class="section">
      ...
      <div class="grid-3">
        ...
        <a href="{{ $report['url'] }}" class="listing-card" style="display:block;">
          <div class="listing-card__body" style="padding:24px;">
            <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:8px;">{{ $report['area'] }}</div>
    ...
    @else
      <p style="color:var(--muted);font-size:15px;margin-bottom:32px;">
        ...
        <a href="{{ route('agent.contact', $agent->slug) }}" style="color:var(--text);text-decoration:underline;">Subscribe to get them by email</a>.
      </p>
      <div style="background:var(--alt);border:1px solid var(--border);border-radius:var(--radius);padding:32px;max-width:600px;">
