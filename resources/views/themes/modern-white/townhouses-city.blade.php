...
<div class="container">
        ...
        <tbody>
          ...
          <tr style="border-top:1px solid var(--border);">
            <td style="padding:12px 16px;font-weight:600;color:var(--text);">
              <a href="{{ route('agent.townhouses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $saSlug]) }}" style="color:var(--text);text-decoration:none;">{{ $saName }}</a>
            </td>
            ...
            <td style="padding:12px 16px;">
              <a href="{{ route('agent.townhouses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $saSlug]) }}" style="color:var(--accent);font-size:12px;text-decoration:underline;white-space:nowrap;">View &rsaquo;</a>
            </td>
          </tr>
          @endforeach
        </tbody>
  ...
  <section class="section" aria-labelledby="listings-heading" style="border-top:1px solid var(--border);padding-top:40px;">
    ...
    @else
    <p style="color:var(--muted);">No active townhouse listings found in {{ $city }} right now. <a href="{{ route('agent.search', $agent->slug) }}" style="color:var(--text);text-decoration:underline;">View all listings</a>.</p>
    @endif
  </section>
