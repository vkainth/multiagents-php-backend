...
{{-- CSS variable: only --accent changes per agent --}}
<style>:root { --accent: {{ $agent->theme_color ?? '#c9a96e' }}; }</style>

<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;800&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">

<link rel="stylesheet" href="{{ asset('css/themes/classic-dark.css') }}?v=2">
...
<body data-theme="{{ $agentTheme ?? 'classic-dark' }}">
...
<nav class="agent-nav" role="navigation" aria-label="Main navigation">
  <a href="{{ route('agent.home', $agent->slug) }}" class="agent-nav__brand" aria-label="{{ $agent->name }} home">
    @if($agent->logo_path)
      <img src="{{ asset($agent->logo_path) }}" alt="{{ $agent->name }}" style="height:38px; width:auto;">
    @else
      <div class="agent-nav__avatar">
        @if($agent->photo_path)
          <img src="{{ asset($agent->photo_path) }}" alt="{{ $agent->name }}">
        @else
          <div style="width:100%;height:100%;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#1a1a1a;font-weight:800;font-size:16px;">{{ substr($agent->name,0,1) }}</div>
        @endif
      </div>
      <div>
  ...
  <ul class="agent-nav__links" id="agent-nav-links">
    ...
    <li><a href="{{ route('agent.about', $agent->slug) }}">About</a></li>
    <li><a href="{{ route('agent.contact', $agent->slug) }}">Contact</a></li>
  </ul>
  ...
  <div class="agent-nav__actions">
    ...
    @endif
    <a href="{{ route('agent.home-evaluation', $agent->slug) }}" class="btn-cta">Book a Showing</a>
  </div>
...
{{-- Mobile nav drawer --}}
<div id="mobile-nav" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:190;" role="dialog" aria-modal="true" aria-label="Mobile navigation">
  <div style="position:absolute;top:0;right:0;width:280px;height:100%;background:#1a1a1a;padding:24px 28px;overflow-y:auto;">
    <button id="mobile-nav-close" style="background:none;border:none;color:#fff;font-size:24px;cursor:pointer;margin-bottom:24px;display:block;">×</button>
    <ul style="list-style:none;">
      @foreach([['Search', route('agent.search', $agent->slug)], ['Sold', route('agent.sold', $agent->slug)], ['Houses', route('agent.houses', $agent->slug)], ['Townhouses', route('agent.townhouses', $agent->slug)], ['Market', route('agent.market-stats', $agent->slug)], ['Neighbourhoods', route('agent.neighbourhoods', $agent->slug)], ['Buyers Guide', route('agent.buyers-guide', $agent->slug)], ['Sellers Guide', route('agent.sellers-guide', $agent->slug)], ['About', route('agent.about', $agent->slug)], ['Contact', route('agent.contact', $agent->slug)]] as [$label, $href])
      <li style="border-bottom:1px solid rgba(255,255,255,0.08);padding:14px 0;">
        <a href="{{ $href }}" style="color:rgba(255,255,255,0.8);font-size:16px;font-weight:600;display:block;">{{ $label }}</a>
      </li>
      @endforeach
    </ul>
    @if($agent->phone)
      <a href="tel:{{ $agent->phone }}" style="display:block;margin-top:24px;padding:12px 20px;background:var(--accent);color:#1a1a1a;text-align:center;border-radius:4px;font-weight:700;font-size:14px;">{{ $agent->phone }}</a>
    @endif
  </div>
</div>
  ...
  <div class="agent-footer__grid">
    ...
    <div>
      <div class="agent-footer__brand">
        <div class="agent-footer__avatar">
          @if($agent->photo_path)
            <img src="{{ asset($agent->photo_path) }}" alt="{{ $agent->name }}">
          @else
            <div style="width:100%;height:100%;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#1a1a1a;font-weight:800;font-size:20px;">{{ substr($agent->name,0,1) }}</div>
          @endif
        </div>
        <div>
      ...
      <div class="agent-footer__contact">
        ...
        @endif
        @if($agent->email)
          <div><a href="mailto:{{ $agent->email }}" style="color:rgba(255,255,255,0.55);">{{ $agent->email }}</a></div>
        @endif
      </div>
    ...
    <div class="w4-sticky__agent">
      <div class="w4-sticky__avatar">
        @if($agent->photo_path)
          <img src="{{ asset($agent->photo_path) }}" alt="{{ $agent->name }}">
        @else
          <div style="width:100%;height:100%;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#1a1a1a;font-weight:800;">{{ substr($agent->name,0,1) }}</div>
        @endif
      </div>
      <div>
