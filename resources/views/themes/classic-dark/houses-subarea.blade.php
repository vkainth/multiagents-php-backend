  ...
  <div class="container">
    <nav aria-label="breadcrumb" style="margin-bottom:10px;">
      <ol style="list-style:none;padding:0;margin:0;display:flex;flex-wrap:wrap;gap:6px;font-size:13px;color:rgba(255,255,255,.6);">
        ...
        <li style="margin:0 4px;">›</li>
        <li><a href="{{ route('agent.houses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:rgba(255,255,255,.6);">{{ $city }}</a></li>
        <li style="margin:0 4px;">›</li>
...
<div class="container">
  ...
  <section class="section--sm" style="padding-top:36px;">
    <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:6px;padding:20px;font-size:14px;color:rgba(255,255,255,.5);margin-bottom:32px;">
      ...
      <a href="{{ route('agent.houses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:var(--accent);margin-left:8px;">View {{ $city }} house market →</a>
    </div>
  </section>
  @endif
  ...
  <section class="section" aria-labelledby="listings-heading" style="border-top:1px solid rgba(255,255,255,.07);padding-top:40px;">
    ...
    <p style="color:var(--muted);">No active house listings found in {{ $subarea }} right now.
      <a href="{{ route('agent.houses.city', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug]) }}" style="color:var(--accent);">Browse all {{ $city }} houses</a>.
    </p>
    @endif
  </section>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
      ...
      <a href="{{ route('agent.houses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $nbSlug]) }}" style="display:block;text-decoration:none;">
        <div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:14px 18px;transition:border-color .2s;">
          ...
          <div style="font-size:12px;color:var(--accent);margin-top:4px;">View houses &rsaquo;</div>
        </div>
      </a>
      @endforeach
    </div>
