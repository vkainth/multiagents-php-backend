...
<div class="container">
  <section class="section">
    <div class="grid-2" style="gap:64px;align-items:start;">
      <div>
        <h2 class="h2 mb-16">Why an accurate valuation matters</h2>
        ...
        <div style="margin-bottom:32px;">
          <div style="font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:16px;">What you receive</div>
        ...
        @if($agent->photo_path)
        <div style="display:flex;align-items:center;gap:16px;padding:16px;background:var(--alt);border-radius:var(--radius);">
          <div style="width:56px;height:56px;border-radius:50%;overflow:hidden;border:2px solid var(--accent);flex-shrink:0;">
            <img src="{{ asset($agent->photo_path) }}" alt="{{ $agent->name }}" style="width:100%;height:100%;object-fit:cover;">
          </div>
          <div>
            <div style="font-weight:700;font-size:14px;">{{ $agent->name }}</div>
            <div style="color:var(--accent);font-size:12px;">{{ $agent->brokerage }}</div>
            @if($agent->phone)
