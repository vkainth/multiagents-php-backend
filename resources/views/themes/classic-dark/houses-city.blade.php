...
<style>
.acd-faq-item{border:1px solid rgba(255,255,255,.1);border-radius:6px;margin-bottom:8px;overflow:hidden;}
.acd-faq-question{padding:13px 16px;cursor:pointer;font-weight:600;font-size:14px;display:flex;justify-content:space-between;align-items:center;background:rgba(255,255,255,.04);color:#fff;user-select:none;}
.acd-faq-question:hover{background:rgba(255,255,255,.08);}
.acd-faq-chevron{font-size:12px;transition:transform .2s;color:rgba(255,255,255,.4);}
.acd-faq-answer{display:none;padding:13px 16px;font-size:13.5px;line-height:1.65;color:rgba(255,255,255,.7);border-top:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.02);}
  ...
  <div class="container">
    <nav aria-label="breadcrumb" style="margin-bottom:10px;">
      <ol style="list-style:none;padding:0;margin:0;display:flex;flex-wrap:wrap;gap:6px;font-size:13px;color:rgba(255,255,255,.6);">
        ...
        <li><a href="{{ route('agent.houses', $agent->slug) }}" style="color:rgba(255,255,255,.6);">Houses</a></li>
        <li style="margin:0 4px;">›</li>
...
<div class="container">
  ...
  {{-- FAQ SECTION --}}
  <section class="section" aria-labelledby="faq-heading" style="border-top:1px solid rgba(255,255,255,.07);padding-top:40px;margin-bottom:8px;">
      ...
      <div class="acd-faq-answer">
        ...
        <dl>
          ...
          <dd>${{ number_format($cond['avg_sold_30d']) }}
            ...
            <span style="color:rgba(255,255,255,.4);font-size:12px;"> — last updated {{ date('F j, Y') }}</span>
          </dd>
        </dl>
        ...
        <tbody>
          ...
          <tr style="border-top:1px solid rgba(255,255,255,.05);">
            <td style="padding:12px 16px;font-weight:600;color:#fff;">
              <a href="{{ route('agent.houses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $saSlug]) }}" style="color:#fff;text-decoration:none;">{{ $saName }}</a>
            </td>
            ...
            <td style="padding:12px 16px;">
              <a href="{{ route('agent.houses.subarea', ['agentSlug' => $agent->slug, 'citySlug' => $citySlug, 'subareaSlug' => $saSlug]) }}" style="color:var(--accent);font-size:12px;white-space:nowrap;">View &rsaquo;</a>
            </td>
          </tr>
          @endforeach
        </tbody>
  ...
  <section class="section" aria-labelledby="listings-heading" style="border-top:1px solid rgba(255,255,255,.07);padding-top:40px;">
    ...
    @else
    <p style="color:var(--muted);">No active house listings found in {{ $city }} right now. <a href="{{ route('agent.search', $agent->slug) }}" style="color:var(--accent);">View all listings</a>.</p>
    @endif
  </section>
